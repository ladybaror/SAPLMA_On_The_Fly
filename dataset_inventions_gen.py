...
# Read the CSV file
df = pd.read_csv("resources\\inventions.csv", encoding='latin1')

#Gotlib was an inventor. That person live in Germany. That person invented a barometer.
dataset = []
...
for i, row in df.iterrows():
    ...
    # Atomic number
    is_a = "  was an inventor."
      

    # Country
    headquarter = " That person invented a "
    sentence = name + is_a+ headquarter + str(invention) + "."
    
    dataset.append((sentence, 1))

    sentence = name + is_a+ headquarter +  get_other("Invention", invention) + "."
...
dataset_df.to_csv("resources\\inventions_it_is_pairs.csv", index=False) #, header=False)