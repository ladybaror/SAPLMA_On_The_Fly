...
device = "cuda"  # or "cpu"
layer = -12
list_of_datasets = ["companies","capitals", "elements","animals"] #["animals","animals_heb","capitals","capitals_heb","cities_heb","colors","colors_heb","companies","companies_heb","elements","elements_heb","foods_heb","inventions","inventions_heb","movies","movies_heb"]#["uncommon"]#["generated"] #
...
for dataset_to_use in list_of_datasets:
    ...
    for i, row in df.iterrows():
        ...
        # Get token by index (e.g., index 2)
            ...
            try:
                token_id = int(row[token_columns[idx]])
                if(idx==2):
                    token_id=-1
                ...
                continue 
            last_hidden_state = outputs.hidden_states[0][layer][0][token_id] #[first_generated_word][layer][batch][input_words_for_first_generated_word_only]#last hidden state of first generated word
            sent = sentences[idx]
            if sent[-1] not in ['.', '..']:
                sent = sent+'.'
            dfs[layer].at[j,'statement'] = sent
            dfs[layer].at[j,'label'] = row[label_columns[idx]]
    ...
    dfs[layer].to_csv("embeddings_3_sentences\\3_sentence_with_tokens_" +  dataset_to_use +"_Smollm_1.7B_" + str(abs(layer)) + ".csv", index=False)
