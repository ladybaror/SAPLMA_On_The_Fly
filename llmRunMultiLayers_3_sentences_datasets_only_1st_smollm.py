...
for dataset_to_use in list_of_datasets:
    ...
    for i, row in df.iterrows():
        ...
        first_token = int(row["token_1"])
        last_hidden_state = outputs.hidden_states[0][layer][0][first_token] #[first_generated_word][layer][batch][input_words_for_first_generated_word_only]#last hidden state of first generated word
        sent = sentences[0]
        ...
        dfs[layer].at[j,'label'] = row['label_1']
    ...
    dfs[layer].to_csv("embeddings_3_sentences\\3_sentence_only_1st_" +  dataset_to_use +"_Smollm_1.7B_" + str(abs(layer)) + ".csv", index=False)
